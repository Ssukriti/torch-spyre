...
graph():
    ...
    %broadcast : [num_users=1] = call_function[target=torch.ops._c10d_functional.broadcast](args = (%y, 0, default), kwargs = {})
    %wait : [num_users=1] = call_function[target=torch.ops._c10d_functional.wait_tensor](args = (%broadcast,), kwargs = {})
    %z : [num_users=1] = call_function[target=operator.mul](args = (%wait, 2), kwargs = {})
    return (z,)
...
graph():
    %l_t_ : torch.Tensor [num_users=1] = placeholder[target=L_t_]
    %y : [num_users=1] = call_function[target=operator.add](args = (%l_t_, %l_t_), kwargs = {})
    %broadcast : [num_users=1] = call_function[target=torch.ops.spyre.broadcast](args = (%y, 0, default), kwargs = {})
    %z : [num_users=1] = call_function[target=operator.mul](args = (%broadcast, 2), kwargs = {})
    return (z,)

## Expected output
All ranks should receive the broadcasted value from root (42 * 2 * 2 = 168)
"""
