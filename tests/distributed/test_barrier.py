...
class TestBarrier(TestCase):
    ...
    def test_barrier_with_computation(self):
        """
        Test barrier with actual computation between ranks.
        Simulates a realistic scenario where ranks do work then synchronize.
        """
        # Each rank creates a tensor and does some computation
        x = torch.ones(100, dtype=torch.float32, device=DEVICE)
        x = x * (self.comm_rank + 1)

        # Simulate some computation time (rank-dependent)
        time.sleep(self.comm_rank * 0.2)

        # Synchronize all ranks
        dist.barrier()

        # After barrier, verify tensor still has correct value
        expected = torch.ones(100, dtype=torch.float32) * (self.comm_rank + 1)
        result = x.to("cpu")
        self.assertTrue(
            torch.allclose(result, expected),
            f"Rank {self.comm_rank}: Tensor value changed after barrier",
        )
