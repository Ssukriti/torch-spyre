...
from .ir import FixedTiledLayout, SpyreConstantFallback, SpyreBroadcastAsyncFallback, SpyreWaitWorkFallback
...
def propagate_spyre_tensor_layouts(
    ...
    for op in it:
        if op.is_no_op():
            ...
            op.layouts = [generic_layout(op)]
            op.restick_cost_fn = AnyInNode.from_args()
        elif isinstance(op, SpyreBroadcastAsyncFallback):
            # Broadcast preserves input layout - no special layout handling needed
            op.layouts = [generic_layout(op)]
            op.restick_cost_fn = AnyInNode.from_args()
        elif isinstance(op, SpyreWaitWorkFallback):
            # Broadcast preserves input layout - no special layout handling needed
            op.layouts = [generic_layout(op)]
            op.restick_cost_fn = AnyInNode.from_args()
